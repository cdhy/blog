...
class CreateCategoriesTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('categories', function (Blueprint $table) {
            ...
            $table->tinyInteger('type')->default(1)->comment('分类类型：1分类，2封面，3专题');
            $table->string('template',100)->default('')->comment('分类模版');
            $table->string('alias',100)->default('')->comment('分类别名');
            $table->string('class',100)->default('')->comment('分类样式');
            $table->string('cover',100)->default('')->comment('分类封面');
